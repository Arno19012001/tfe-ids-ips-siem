...
| Critère | Preuve |
|---|---|
| A1 (alerte SYN scan < 30s) | SID 1000101, déclenchement soutenu sur toute la campagne, premières alertes en quelques secondes |
| A2 (alerte visible dans Wazuh Dashboard) | Alertes 1000101/1000104 confirmées dans `wazuh-alerts-*` (Discover/Dashboard), dashboard dédié exporté en Issue #14 (`wazuh/dashboards/scenario_a_dashboard.ndjson`), revalidé visuellement en direct sur cette campagne |
| A3 (technique T1046 affichée) | SID 1000101/1000104, `mitre_technique: T1046` présent dans les métadonnées de chaque alerte |
| A4 (IP bloquée après seuil) | Active-response validée bout-en-bout (commit `a80d6e3`), désactivée par défaut, réactivable |
