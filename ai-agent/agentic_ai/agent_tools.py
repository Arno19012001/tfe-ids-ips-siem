...
# NOTE (adaptation TFE, 10/08/2026) : l'outil get_event_sequence original de
# octopus237/Agentic-AI a été retiré ici. Il reposait entièrement sur les
# champs data.win.eventdata.* (Sysmon/Windows), absents de cet environnement
# (Linux/Suricata/SSH uniquement, aucun hôte Windows dans le lab). Conserver
# une fonction qui ne renverrait jamais de données aurait été trompeur pour
# le modèle plutôt qu'un simple manque de couverture explicite.
...
SYSTEM_PROMPT = (
    ...
    "alerts:\n"
    "- Use get_agent_timeline on the host in question to see the ordered chain "
    "of events (rule descriptions, MITRE tactics, in chronological order). "
    "Describe the sequence: X led to Y led to Z.\n"
    ...
    "IMPORTANT — LANGUE : rédige TOUJOURS ta réponse finale (le verdict) en "
    "français, quelle que soit la langue de la question posée. Ton raisonnement "
    "intermédiaire (avant les appels d'outils) peut rester en anglais si c'est "
    "plus naturel pour toi, mais le verdict final destiné à l'analyste SOC doit "
