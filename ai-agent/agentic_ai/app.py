...
def _run_agentic(question, run_id, q=None):
    ...
    report = (f"QUESTION : {question}\n\n{final}\n\n"
              f"{'─'*50}\nJOURNAL DES APPELS D'OUTILS ({len(audit)} appels)\n{'─'*50}\n"
              f"{audit_text}")
...
def agent_stream():
    ...
    if not question:
        def bad():
            yield "data: [Aucune question fournie.]\n\n"
            yield "data: __DONE__\n\n"
        return Response(bad(), mimetype="text/event-stream")

    if not ST.lock.acquire(blocking=False):
        def busy():
            yield "data: [Une investigation est déjà en cours. Attendez qu'elle se termine.]\n\n"
            yield "data: __DONE__\n\n"
...
<head>
<meta charset="utf-8">
<title>Analyste Sécurité Agentique</title>
...
<header>
  ...
  <span class="badge" id="status-badge">inactif</span>
  <div class="hdr-tabs">
    <div class="hdr-tab active" id="tab-run-btn" onclick="switchTab('run')">
      <span class="dot"></span>Lancer
    </div>
...
<div class="tab active" id="tab-run">
  <div class="run-tab">
    ...
    <div class="controls">
      <div class="nl-wrap">
        <input id="nl-input" type="text" autocomplete="off" spellcheck="false"
          placeholder="Posez n'importe quelle question — ex. corréler les événements de sévérité 12 sur les 20 derniers jours, ou y a-t-il une exfiltration de données sur mes hôtes ?"
          onkeydown="if(event.key==='Enter')askConfirm()">
        <button class="nl-btn" onclick="askConfirm()">Lancer</button>
      </div>
      <div id="nl-preview"></div>
      <div style="display:flex;gap:6px;align-items:center">
        <button class="btn-stop" id="stop-btn" onclick="stopRun()" disabled>Arrêter</button>
        <p class="hint" style="margin:0">L'agent planifie sa propre investigation, exécute des requêtes Wazuh, et rend un verdict. Cela peut prendre plusieurs minutes.</p>
      </div>
    </div>
    ...
    <div class="sched-bar">
      ...
      <span class="si">Triage automatique toutes les</span>
      <input type="number" id="sched-hours" value="8" min="1" max="72"
        onchange="updateSched()">
      <span class="si">heures — sur les</span>
      <input type="number" id="sched-window" value="24" min="1" max="336"
        onchange="updateSched()">
      <span class="si">heures d'événements</span>
      <span class="si" id="sched-status" style="margin-left:4px">Désactivé</span>
    </div>

    <div class="live-wrap">
      <div class="live-header">
        <div class="live-dot" id="dot"></div>
        <span class="live-title" id="live-title">Sortie</span>
        <span class="elapsed" id="elapsed"></span>
        <button id="copy-live-btn" onclick="copyLive()"
          style="margin-left:auto;font-size:11px;padding:3px 10px;
          background:#21262d;color:#8b949e;border:1px solid #30363d;
          border-radius:4px;cursor:pointer">Copier</button>
      </div>
      <div id="live-out">Prêt. Posez une question ci-dessus et cliquez sur Lancer.</div>
...
<div class="tab" id="tab-reports">
  <div class="reports-tab">
    <div class="reports-sidebar">
      <div class="sb-head">Historique des investigations</div>
      <div class="hist-list" id="hist-list">
        <div style="padding:10px;font-size:11px;color:#484f58">Aucune investigation pour l'instant.</div>
      </div>
    </div>
    <div class="report-main">
      <div class="report-header">
        <span id="report-header-text">Sélectionnez une investigation dans le panneau latéral</span>
...
function askConfirm() {
  ...
  p.innerHTML =
      '<div class="confirm-q">Lancer l\'investigation agentique ?<br><strong>"'
    + esc(q) + '"</strong></div>'
    + '<div class="confirm-note">L\'agent va planifier et exécuter plusieurs '
    + 'requêtes Wazuh. Cela peut prendre plusieurs minutes.</div>'
    + '<button class="confirm-yes" onclick="confirmRun()">Oui, lancer</button>'
    + '<button class="confirm-no" onclick="cancelConfirm()">Annuler</button>';
}
...
function startAgent(question) {
  ...
  _liveBuffer = '';
  _curRunId   = null;
  document.getElementById('live-title').textContent = 'Investigation en cours';
...
function stopRun() {
  ...
  setRunning(false);
  document.getElementById('elapsed').textContent = '';
  document.getElementById('live-title').textContent = 'Sortie — arrêtée';
  document.getElementById('dot').className = 'live-dot';
  const note = document.createElement('div');
  note.style.cssText = 'color:#f0883e;font-size:12px;margin-top:8px';
  note.textContent = '■ Arrêté par l\'utilisateur. Une étape du modèle déjà en '
    + 'cours peut se terminer en arrière-plan, mais son résultat est ignoré.';
...
function finish(err) {
  ...
  document.getElementById('elapsed').textContent = '';
  document.getElementById('live-title').textContent = 'Sortie';
  document.getElementById('last-run').textContent =
    'terminé ' + new Date().toLocaleTimeString();
  _loadHistoryData();
}
...
function setRunning(on) {
  ...
  document.getElementById('status-text').textContent = on ? 'investigation en cours…' : 'inactif';
  document.getElementById('status-badge').textContent = on ? 'en cours' : 'inactif';
...
function statusLabel(s) {
  // Traduit le statut brut (utilisé aussi comme classe CSS .ds.<status> —
  // ne pas modifier la valeur brute, seulement le libellé affiché).
  return {running: 'en cours', completed: 'terminé',
          stopped: 'arrêté', error: 'erreur'}[s] || s;
}

function _renderHistory() {
  const el = document.getElementById('hist-list');
  if (!_histData.length) {
    el.innerHTML = '<div style="padding:10px;font-size:11px;color:#484f58">Aucune investigation pour l\'instant.</div>';
    return;
  }
  el.innerHTML = _histData.map(i => {
    ...
    const copyBtn = (i.report)
      ? '<button class="hbtn" data-copy="' + i.id + '" style="color:#8b949e">Copier</button>' : '';
    ...
    return '<div class="hi' + active + '" data-id="' + i.id + '">'
      ...
      + '<span style="margin-left:auto;color:' + color + '">' + statusLabel(i.status) + '</span>'
...
function showReport(id) {
  ...
  fetch('/history/' + id).then(r => r.json()).then(d => {
    ...
    if (d.status === 'running') {
      hdr.textContent = 'Investigation en cours — voir l\'onglet Lancer pour le suivi en direct';
      out.textContent = 'Le rapport apparaîtra ici une fois l\'investigation terminée.';
      if (cb) cb.style.display = 'none';
    } else {
      hdr.textContent = d.label || d.id;
      out.innerHTML = renderMD(d.report || '[Aucun rapport disponible]');
      out.scrollTop = 0;
...
function deleteInv(id) {
  if (!confirm('Supprimer cette investigation ?')) return;
    ...
    if (d.ok) {
      if (_activeId === id) {
        ...
        document.getElementById('report-header-text').textContent =
          'Sélectionnez une investigation dans le panneau latéral';
...
function updateSched() {
  fetch('/schedule', {
    ...
    document.getElementById('sched-status').textContent =
      d.enabled ? 'Activé — toutes les ' + d.interval_hours + 'h' : 'Désactivé';
  });
}
...
def main():
    ...
    p = argparse.ArgumentParser(description="Analyste Sécurité Agentique Wazuh")
    p.add_argument("command", nargs="?", default="run",
                   choices=["run", "start", "stop", "restart", "status"],
                   help="run=premier plan (défaut), start=arrière-plan, "
                        "stop=arrêter l'arrière-plan, restart, status")
    ...
    if args.command == "stop":
        pid = _read_pid()
        if not pid:
            print("Non démarré (aucun fichier PID actif).")
